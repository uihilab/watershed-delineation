...
def get_data_from_binary(binfile):
	with open(binfile, mode='rb') as file:
		watershed_data = file.read()
...
-y yvalue 		: y value of the target point, should be integer"""
	...
	if inputtype not in ["bin", "png"]:
		_print("Input type should be either png or bin.")
		sys.exit()

	if outputtype not in ["raw", "kml"]:
		_print("Output type should be either raw or kml.")
		sys.exit()

	if inputfile == "":
		_print("Please provide an input file.")
		sys.exit()

	if outputfile == "":
		_print("Please provide an output file name.")
		sys.exit()
